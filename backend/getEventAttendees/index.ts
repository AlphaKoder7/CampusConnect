...
export async function getEventAttendees(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  try {
    ...
    if (event.creatorId !== principal.userId) {
      return {
        ...
        body: JSON.stringify({ error: 'Only event creators can view attendees' })
      };
    }
    ...
    context.error('Error getting event attendees:', error);
    return {
      status: 500,
      headers: {
        'Content-Type': 'application/json',
